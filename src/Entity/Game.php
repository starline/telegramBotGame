...
class Game
{
     ...
     * @return int|bool
     */
    protected function getUserId(string $user_mention = null)
    {
        return $this->getUser($user_mention) ? $this->getUser($user_mention)->getId() : false;
    }
     ...
     * @throws BotException
     */
    protected function getCurrentUser(Bool $raw_data = false)
    {
        ...
        // Object to array. Contains Object name
        $user_object = $update_object->getFrom();
        $user_object_as_array = (array) $user_object;

        $object_as_array =  $user_object_as_array;
        unset($object_as_array['raw_data'], $object_as_array['bot_username']);

        Utilities::isDebugPrintEnabled() && Utilities::debugPrint('current', $object_as_array);

        if ($raw_data === true) {
            return $user_object_as_array['raw_data'];
        }
        return $user_object;
    }
     ...
     * @return string|bool
     */
    protected function getUserMention(String $user_mention = null)
    {
        if (isset($this->data['settings']['use_old_mentions']) && $this->data['settings']['use_old_mentions'] === true) {
            return $this->getUser($user_mention) ? filter_var($this->getUser($user_mention)->tryMention(), FILTER_SANITIZE_SPECIAL_CHARS) : false;
        }
        return $this->getUser($user_mention) ? '<a href="tg://user?id=' . $this->getUser($user_mention)->getId() . '">' . filter_var($this->getUser($user_mention)->getFirstName(), FILTER_SANITIZE_SPECIAL_CHARS) . '</a>' : false;
    }


    /**
     * Get player's user object
     * @param string $user_mention - host|guest
     * @return User|bool
     */
    protected function getUser(String $user_mention = null)
    {
        if (empty($user_mention)) {
            return false;
        }

        $result = isset($this->data['players'][$user_mention]['id']) ? new User($this->data['players'][$user_mention]) : false;

        if ($result !== false) {
            $object_as_array = (array) get_object_vars($result);
            unset($object_as_array['raw_data'], $object_as_array['bot_username']);

            Utilities::isDebugPrintEnabled() && Utilities::debugPrint($user_mention, $object_as_array);
        } else {
            Utilities::isDebugPrintEnabled() && Utilities::debugPrint($user_mention . ' = false');
        }
